...
public class GeometryLoader {
	...
	private void readPositions() {
		...
		for (int i = 0; i < count / 3; i++) {
			...
			vertices.add(new Vertex((short) vertices.size(), new Vector3f(position.x, position.y, position.z),
	...
	private void assembleVertices() {
		...
		for (int i = 0; i < indexData.length / typeCount; i++) {
			short positionIndex = Short.parseShort(indexData[i * typeCount]);
	...
	private Vertex processVertex(short posIndex, int normIndex, int texIndex) {
	...
	private short[] convertIndicesListToArray() {
		this.indicesArray = new short[indices.size()];
	...
	private Vertex dealWithAlreadyProcessedVertex(Vertex previousVertex, int newTextureIndex, int newNormalIndex) {
		if (previousVertex.hasSameTextureAndNormal(newTextureIndex, newNormalIndex)) {
			...
			if (anotherVertex != null) {
				...
				Vertex duplicateVertex = new Vertex((short) vertices.size(), previousVertex.getPosition(),
