...
import com.grillecube.common.world.block.BlockPlant;
import com.grillecube.common.world.terrain.WorldObjectTerrain;

public class TerrainMesherMarchingCubes extends TerrainMesher {
	...
	public TerrainMesherMarchingCubes(int lod) {
		this(lod, lod, lod);
	}

	public TerrainMesherMarchingCubes(int lodx, int lody, int lodz) {
		this.lod = new Vector3i(Maths.clamp(lodx, 1, WorldObjectTerrain.DIMX), Maths.clamp(lody, 1, WorldObjectTerrain.DIMY),
				Maths.clamp(lodz, 1, WorldObjectTerrain.DIMZ));
	}

	public TerrainMesherMarchingCubes() {
		this(1);
	}
...
/*V*/				{new T(Face.BOT, 0, 3, 8, 1, 0, 3)},
	...
	protected void fillVertexStacks(WorldObjectTerrain terrain, ArrayList<TerrainMeshTriangle> opaqueStack,
		...
		for (int z = -1; z <= WorldObjectTerrain.DIMZ; z += this.lod.z) {
			for (int y = -1; y <= WorldObjectTerrain.DIMY; y += this.lod.y) {
				for (int x = -1; x <= WorldObjectTerrain.DIMX; x += this.lod.x) {
	...
	private void processBlock(ArrayList<TerrainMeshTriangle> opaqueStack,
			ArrayList<TerrainMeshTriangle> transparentStack, WorldObjectTerrain terrain, int x, int y, int z) {
		...
		if (block instanceof BlockPlant) {
			blockRenderer.generateBlockVertices(new TerrainMesherCull(), terrain, block, x, y, z, null,
					transparentStack);
			return;
		}
		...
		for (int edgeID = 0; edgeID < 12; edgeID++) {
			...
			if ((edgeFlags & (1 << edgeID)) != 0) {
				...
				float posx = x + (BlockRenderer.VERTICES[v0].x + offx) * this.lod.x + WorldObjectTerrain.BLOCK_DEMI_SIZE;
				float posy = y + (BlockRenderer.VERTICES[v0].y + offy) * this.lod.y + WorldObjectTerrain.BLOCK_DEMI_SIZE;
				float posz = z + (BlockRenderer.VERTICES[v0].z + offz) * this.lod.z + WorldObjectTerrain.BLOCK_DEMI_SIZE;
	...
	private TerrainMeshVertex generateVertex(WorldObjectTerrain terrain, int x, int y, int z, T t, int edgeID,
