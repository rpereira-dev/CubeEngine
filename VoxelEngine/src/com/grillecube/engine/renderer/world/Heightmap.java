...
public class Heightmap {
	...
	public void generateVertices(float[][] heights) {
		...
		FloatBuffer buffer = BufferUtils.createFloatBuffer(this._vertex_count * FLOAT_PER_VERTICES);
		...
		for (int x = 0 ; x < this._width - 1 ; x++) {
			for (int z = 0 ; z < this._height - 1 ; z++) {
				...
				buffer.put(x);
				buffer.put(heights[x][z]);
				buffer.put(z);
				
				buffer.put(x + 1);
				buffer.put(heights[x + 1][z]);
				buffer.put(z);
				
				buffer.put(x);
				buffer.put(heights[x][z + 1]);
				buffer.put(z + 1);
