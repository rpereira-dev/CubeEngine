...
public class ActionListenerExportModelBuilder implements ActionListener {
	...
	public ActionListenerExportModelBuilder(ModelEditor window) {
		this._editor = window;
	}
	...
	public void actionPerformed(ActionEvent event) {
		...
		if (chooser.showSaveDialog(null) == JFileChooser.APPROVE_OPTION) {
			...
			ModelEditor.instance().getEngine().getRenderer().addGLTask(new GLTask() {
				@Override
				public void run() {
					ModelBuilderExporter.exportModelBuilder(_editor.getModel(), filepath);
					ModelEditor.instance().getEngine().getRenderer().getGuiRenderer().toast("Model was saved!", 0.0f,
							1.0f, 0.0f, 1.0f);
				}
			});
		}
